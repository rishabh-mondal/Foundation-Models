...
• Loss:  L = L_det^S + λ_unsup * L_det^T + λ_sim * L_sim
    ...
    - L_sim   : cosine patch-alignment to a frozen DINO encoder
        · Source aligned from iter 0
        · Target added after ninitSim

Directory layout (per region):
  <root>/<split>/images
  <root>/<split>/labels                    # GT for evaluation (if available)
  <root>/<split>/pseudo_fused_labels       # offline pseudo-labels (.txt, YOLO xywh normalized)

Example:
  python phase2_train_eval.py \
    --src_root /.../uttar_pradesh --src_split train \
    --tgt_roots /.../bangladesh /.../pak_punjab --tgt_split train \
    --val_root /.../uttar_pradesh --val_split val \
    --eval_tgt_roots /.../bangladesh /.../pak_punjab --eval_tgt_split test \
    --dinov3_dir /.../dinov3 --dino_model dinov3_vitl16 \
    --dino_weights /.../dinov3_vitl16_pretrain_sat493m-*.pth \
    --epochs 12 --batch_size 8 --image_size 800 \
    --use_ema_teacher --ema_start_iter 20000 --ninitPL 20000 --ninitSim 5000 \
    --save_dir ./phase2_runs
"""
...
def yolo_txt_to_xyxy(txt_path: Path, W: int, H: int) -> Tuple[List[List[float]], List[int]]:
    """Read YOLO (xywh normalized) .txt -> XYXY boxes + labels(+1)."""
    boxes, labels = [], []
    ...
    with open(txt_path, "r") as f:
        for line in f:
            ...
            if len(p) != 5:  # cls cx cy w h
                continue
            c = int(float(p[0])) + 1  # shift to 1..K (0=bg reserved)
...
class SourceDetDataset(Dataset):
    """Source with GT labels (YOLO-OBB 9 cols or YOLO xywh 5 cols)."""
        ...
        if txt.exists():
            with open(txt, "r") as f:
                for line in f:
                    ...
                    elif len(parts) == 5:  # xywh normalized
...
class TargetDetDataset(Dataset):
    """Target with optional offline pseudo labels (xywh normalized)."""
...
class FeatProjector(nn.Module):
    ...
    def forward(self, f):  # [B,C,H,W] -> [B, D, H, W] (L2-normalized)
        z = self.net(f)
...
def evaluate_region(model, root: str, split: str, device,
    ...
    base = Path(root) if (Path(root) / "images").is_dir() else (Path(root) / split)
    ds = SourceDetDataset(root, split, image_size)  # uses GT labels in <split>/labels
...
# ----------------------------
def parse_args():
    ap = argparse.ArgumentParser("Phase-2 Train/Val/Eval with DINOv3 Faster R-CNN")
    # Data (train/val)
    ...
    # Eval on target
    ap.add_argument("--eval_tgt_roots", nargs="+", default=[], help="target roots for final evaluation")
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        for imgs_s, tgts_s in pbar:
            ...
            # Weak/Strong from disk paths (teacher weak, student both)
            try:
            ...
            # 1) Supervised det on source (weak+strong)
        ...
        if val_map > best_map:
            ...
            print(f"[CKPT] saved -> {ckpt_path} (mAP50={best_map:.4f})")
